...
impl ScanJob {
    async fn run(&mut self, directories: Vec<String>) -> Option<()> {
        // 文件读取
    ...
    async fn save_basket(&mut self, session: &Session) -> Option<()> {
    ...
    async fn load_dir(&mut self, directories: Vec<String>) {
        let timer = Timer::start();

        directories.iter().map(|v| Path::new(v)).for_each(|v| {
            info!("<scan:{}> 路径：{:?}", self.id, v.as_os_str());
            self.load_file_list(v).print_error();
        });

        self.send_msg("scanner".to_string(), self.file_count.to_string())
            .await;
        info!(
    ...
    async fn save_folder(&mut self, session: &Session) -> Option<()> {
        let timer = Timer::start();
        ...
        for folder in self.folder_list.iter_mut() {
            if !folder.exist(&session).await {
                ...
                let current = Folder::get_by_path(&session, &folder.path).await?;
                let parent = Folder::get_by_path(&session, get_path_parent(&folder.path)?).await?;
                if current.pid == 0 {
    ...
    async fn load_task(&mut self, session: &Session) {
        let timer = Timer::start();
        ...
        self.send_msg("task".to_string(), self.task_count.to_string())
            .await;
        info!(
    ...
    async fn run_scanner(&mut self, session: &Session) -> Option<()> {
        let timer = Timer::start();
        ...
        self.send_msg("done".to_string(), self.scan_count.to_string())
            .await;
        info!(
    ...
    async fn send_msg(&mut self, r#type: String, data: String) {
        self.tx.send(ScanMsg::new(r#type, data)).await.print_error();
    }
    ...
    fn is_support(&self, path: &Path) -> bool {
        match get_file_suffix(path) {
            Some(suffix) => self.scanners.iter().any(|v| v.is_support(suffix)),
            None => false,
        }
    }
}
...
fn is_hidden(path: &Path) -> bool {
    || -> Option<bool> { Some(path.file_name()?.to_str()?.starts_with('.')) }()
...
/// 获取父级路径
fn get_path_parent(path: &str) -> Option<&str> {
